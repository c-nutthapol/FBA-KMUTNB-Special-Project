   ...
   <div>
       @livewire('students.project.components.header', ['project' => $project])
       @if ($data->error)
           <div class="-mx-3 flex flex-wrap">
               <div class="w-full max-w-full flex-none px-3">
                   <div
                       ...
                       <div class="p-6 text-center text-xl text-red-500">
                           <div>ไม่สามารถสร้างโครงงานใหม่ได้ เนื่องจากท่านมีโครงงานที่สร้างไว้แล้ว
                           </div>
                           <div>หากต้องการแก้ไขข้อมูลกรุณาใช้งานเมนู "สร้างคำร้องทั่วไป"
                               เพื่อขอแก้ไขข้อมูลโครงงาน</div>
                       </div>
                       <div class="p-6 text-center">
                           <a href="{{ route('student.project.home') }}">
                               <button class="btn btn-primary">ย้อนกลับ</button>
                           </a>
                           <a href="{{ route('student.petition') }}">
                               <button class="btn btn-primary">สร้างคำร้องทั่วไป</button>
                           </a>
                       </div>
                   </div>
               </div>
   ...
   @push('script')
       <script>
           ...
           s2.on('change', function(e) {
               @this.set('form.student_2.id', s2.val())
           })
           t1.on('change', function(e) {
               @this.set('form.teacher_1', t1.val())
           })
           t2.on('change', function(e) {
               @this.set('form.teacher_2', t2.val())
           })
           t3.on('change', function(e) {
               @this.set('form.teacher_3', t3.val())
           })
       </script>
   @endpush

...
class Suggestion extends Component
{
    ...
    {
        try{
            $model = Master_suggestion::where('id',$this->t_id)->first();
            $model->update([
                'name'=> $this->t_name,
    ...
    {
        try{
            $model = Master_suggestion::where('id',$id)->first();
            $model->delete();
    ...
    {
        $data = Master_suggestion::query()
        ->where('name', 'like', '%' . $this->search . '%')
        ->paginate(10);
        return view('livewire.admin.settings.master.suggestion')->with(compact('data'));
    }
}

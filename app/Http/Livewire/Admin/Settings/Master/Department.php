...
class Department extends Component
{
    ...
    {
        try{
            $model = Master_department::where('id',$this->t_id)->first();
            $model->update([
                'name'=> $this->t_name,
    ...
    {
        try{
            $model = Master_department::where('id',$id)->first();
            $model->delete();
    ...
    {
        $data = Master_department::query()
        ->where('name', 'like', '%' . $this->search . '%')
        ->paginate(10);
        return view('livewire.admin.settings.master.department')->with(compact('data'));
    }
}
